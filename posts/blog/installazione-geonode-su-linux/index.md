...
Siamo ora pronti per installare una serie di pacchetti e librerie, necessari alla stessa installazione di Geonode. Anche in questo caso forniamo la risposta di default `-y` per velocizzarne il processo.
...
Nel comando appena inserito i vari pacchetti sono elencati separati da spazio. Tra gli altri, il comando sopra installerà:
...
A questo punto siamo pronti per installare le dipendenze di Docker, tra le quali anche [git](../git-controllo-di-versioni-1/) e alcuni pacchetti per la sicurezza dell'installazione:
...
Si è quasi pronti a scaricare Docker per Ubuntu dal [sito ufficiale](https://download.docker.com/linux/ubuntu/gpg); è solo necessario prima scaricare e installare una chiave crittografica di verifica che assicura che il pacchetto che verrà installato è esattamente quello ufficialmente rilasciato. Si userà [cURL](https://curl.se/) per scaricare la chiave e `apt-key add` per installarla:
...
A differenza della guida ufficiale, che potrete trovare a questo [link](https://docs.geonode.org/en/master/install/advanced/core/index.html#docker), l'installazione qui descritta prevede l’aggiornamento di `Docker Compose` alla versione 1.29. [Docker Compose](https://docs.docker.com/get-started/08_using_compose/) è uno strumento sviluppato per definire e condividere applicazioni composte da molti _container_:
...
In dettaglio i comandi sopra permettono di:
...
- si cambiano alla directory `opt/geonode` i permessi in `775`, che significa che proprietario e membri del gruppo principale possono leggere, scrivere ed eseguire i file della cartella, mentre gli altri utenti possono solo leggere ed eseguire.
...
Geonode è abilitato per fare da `OAuth Provider` per certificare la veridicità dell'identità dell'utente, permettendo così al sistema di lavorare in un ambiente isolato e consentire al software stesso di autenticare utenti privati gestiti dal sottosistema di autenticazione Django.
...
Una volta finito di modificare il file `.env` è necessario riavviare i servizi, affinchè le nuove configurazioni vengano recepite:
...
È possibile eseguire dei controlli per verificare che tutti i servizi installati e attivati stiano funzionando correttamente.
...
Maggiori informazioni sul comando `docker-compose logs` sono disponibili nella guida ufficiale, all'indirizzo: [https://docs.docker.com/engine/reference/commandline/compose_logs/](https://docs.docker.com/engine/reference/commandline/compose_logs/).
