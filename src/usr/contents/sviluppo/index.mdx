...
Installazione e configurazione sono semplici: è possibile installare l'estensione via NPM (`npm install @lad-sapienza/directus-extension-zotero-sync`) oppure copiarne manualmente `package.json` e la cartella `dist` nella directory `extensions/directus-extension-zotero-sync/` dell'istanza Directus e riavviare il servizio. Durante lo sviluppo è possibile abilitare il reload automatico delle estensioni e usare una modalità di debug per eseguire sync manuali.
