...
The first version handled the essential use case well — give me a PDF, I return the place names — but it showed its limits when used on real work with large bibliographic volumes and with archaeological documentation. The revision addresses four fronts.
...
The revision added a suite of automated tests (`pytest`) that verify, offline and without downloading models, the behaviour of the pure functions: parsing of page ranges, gazetteer matching, reading files in various formats, serialisation. It is a test of this kind that would have caught, at the time, the error in the gazetteer's regular expression.
...
geoNamesFromPdf remains a deliberately narrow tool: it does one thing — find the places in a PDF — and tries to do it well, while staying simple to install and to use. The 2026 revision, however, also makes it suitable for work at a larger scale: processing of entire volumes, output that integrates with GIS and databases, gazetteers that carry coordinates, and a mode designed for ancient languages. All of this without giving up offline operation and without depending on external services.
